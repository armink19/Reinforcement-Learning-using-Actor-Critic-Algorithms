...
def intrinsic_reward(state, next_state):
    # Extract relevant information from the states
    gripper_pos = state['gripper_pos']
    ...
    # Step-wise rewards based on different actions
    rewards = 0
    if gripper_object_distance < 0.02:  # Close to the object
        rewards += 0.1
    if gripper_object_distance < 0.01:  # Touching the object
        rewards += 0.1
    if np.all(next_gripper_pos == target_pos):  # Placing in target position
        rewards += 1.0
    if state['obj_pos']!=next_state['obj_pos']:  # Picking the object
        rewards += 0.5
    if action[0] == 5:  # Moving the object without losing
        rewards += 0.2
    ...
    with tf.GradientTape() as actor_tape:
        with tf.GradientTape() as critic_tape:
            for step in range(MAX_STEPS):
                ...
                intrinsic_rewards = intrinsic_reward(state, next_state)

                # Combined reward
                combined_rewards = reward + intrinsic_rewards

                action_probs.append(tf.math.log(action_prob))
                critic_values.append(critic_value)
                rewards_history.append(combined_rewards)
                distances.append(distance)

                # Update the current state and episode reward
                state = next_state
                episode_reward += combined_rewards
